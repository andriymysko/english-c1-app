...
export default function GrammarList({ onSelectTopic, onPremiumLocked }: Props) {
    ...
    const handleSelect = (topic: any) => {
        // Bloquejat SI: és Premium I l'usuari NO és VIP
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {TOPICS.map(topic => {
                // 1. CALCULEM L'ESTAT FORA DEL JSX PER CLARETAT
                const isLocked = topic.isPremium && !user?.is_vip;

                // 2. DECIDIM ELS ESTILS EN FUNCIÓ DE SI ESTÀ TANCAT O NO
                const cardStyle = isLocked 
                    ? "bg-gray-100 border-gray-300 opacity-80" // ESTIL TANCAT (Gris i apagat)
                    : `bg-white ${topic.border} hover:shadow-xl hover:scale-[1.02] hover:border-transparent`; // ESTIL OBERT
